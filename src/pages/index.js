...
const openPopupPlaceAdd = new PopupWithForm(popupCard, {
    submitForm: (item) => {
        api.postUserCard(item)
            ...
            .finally(() => {
                openPopupPlaceAdd.loaderHandler('Сохранить...');
            })
    }
})
        ...
        handleRemoveCard: () => {
            ...
            openPopupConfirm.setHandleSubmit(() => {
                    api.deleteCard(card._id)
                        .then(() => {
                            card.removeCard();
                            openPopupConfirm.close();
                        })
                        .catch((err) => {
                            console.log(err);
                        })
                        .finally(() => {
                            openPopupConfirm.loaderHandler('Удалить...');
                        })
                }
            );
        },
...
const openPopupAvatar = new PopupWithForm(popupAvatar, {
    submitForm: (item) => {
        api.patchAvatar(item)
            ...
            .finally(() => {
                openPopupAvatar.loaderHandler('Сохранить...');
            })
    }
})
...
const profileForm = new PopupWithForm(popupProfile, {
    submitForm: (item) => {
        api.patchUserProfile(item)
            ...
            .finally(() => {
                profileForm.loaderHandler('Сохранить...');
            })
    }
})
